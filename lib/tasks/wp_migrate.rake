...
namespace :wordly do
  ...
  namespace :wordpress do
    ...
    task :migrate => :environment do
      ...
      Rake::Task['wordly:wordpress:migrate_users'].execute({:conn => conn})
      Rake::Task['wordly:wordpress:migrate_posts'].execute({:conn => conn})
      Rake::Task['wordly:wordpress:migrate_pages'].execute({:conn => conn})
      Rake::Task['wordly:wordpress:migrate_categories'].execute({:conn => conn})
      Rake::Task['wordly:wordpress:migrate_comments'].execute({:conn => conn})
      puts "\nDone!\n"
    end

    desc "Migrate Wordpress users to Wordly"
    task :migrate_users, [:conn] => :environment do |t,args|
      ActiveRecord::Base.connection.execute('truncate table authors')
      client = Mysql2::Client.new(args[:conn])
      puts "\nMigrating Users"
    ...
    desc "Migrate Wordpress posts to Wordly"
    task :migrate_posts, [:conn] => :environment do |t,args|
      puts "\nMigrating posts"
    ...
    desc "Migrate Wordpress pages to Wordly"
    task :migrate_pages, [:conn] => :environment do |t,args|
      puts "\nMigrating pages"
    ...
    desc "Migrate Wordpress categories and posts categories relationships to Wordly"
    task :migrate_categories, [:conn] => :environment do |t, args|
    ...
    desc "Migrate Wordpress comments to Wordly"
    task :migrate_comments, [:conn] => :environment do |t,args|
      puts "\nMigrating comments"
    ...
    desc "Migrate Wordpress tags to Wordly"
    task :migrate_tags, [:conn] => :environment do |t, args|
      puts "\nMigrating post tags"
      client = Mysql2::Client.new(args[:conn])

      q = "SELECT wt.name, wtr.object_id From wp_terms wt
